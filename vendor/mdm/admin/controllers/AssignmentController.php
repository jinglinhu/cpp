...
class AssignmentController extends Controller
{
    ...
    public function actionView($id)
    {
        ...
        foreach ($authManager->getRolesByUser($id) as $role) {
            $type = $role->type;
            $assigned[$type == Item::TYPE_ROLE ? '角色' : 'Permissions'][$role->name] = $role->description;
        }
        foreach ($authManager->getRoles() as $role) {
            if (!isset($assigned['角色'][$role->name])) {
                $avaliable['角色'][$role->name] = $role->description;
            }
        }
        foreach ($authManager->getPermissions() as $role) {
            if ($role->name[0] !== '/' && !isset($assigned['Permissions'][$role->name])) {
                $avaliable['Permissions'][$role->name] = $role->description;
            }
        }
    ...
    public function actionRoleSearch($id, $target, $term = '')
    {
        ...
        $var = ${$target};
        if (!empty($term)) {
            foreach (['角色', 'Permissions'] as $type) {
                if (isset($var[$type])) {
